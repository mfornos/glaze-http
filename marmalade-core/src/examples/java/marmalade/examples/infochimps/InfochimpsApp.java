...
public class InfochimpsApp
{
   ...
   private void influence(String screenName)
   {
      ...
      InfluenceMetrics metrics = Get(uri).with(infochimpsErrors()).map(InfluenceMetrics.class);
   ...
   private void tsrank(String screenName)
   {
      ...
      Map<String, Object> result = Get(uri).with(infochimpsErrors()).map();
   ...
   private void ufoSight(String query)
   {
      ...
      UfoSights sights = Get(uri).with(infochimpsErrors()).map(UfoSights.class);
